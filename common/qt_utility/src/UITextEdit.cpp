...
UITextEdit::UITextEdit(QWidget *parent) : QTextEdit(parent) {
}


bool UITextEdit::canInsertFromMimeData(const QMimeData *source) const {
    return source->hasImage() || QTextEdit::canInsertFromMimeData(source);
}


void UITextEdit::insertFromMimeData(const QMimeData *source) {
    if (source->hasImage()) {
...
QMimeData *UITextEdit::createMimeDataFromSelection() const {
    return QTextEdit::createMimeDataFromSelection();
}


void UITextEdit::dropImage(const QImage& image, const QString& format) {
    QByteArray bytes;
